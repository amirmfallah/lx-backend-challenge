...
function CalculateSuperannuation(salary) {
  return AtoRound(decimalUtils.times(configs.superannuationRate, salary), 0.01);
}

function CalculateMedicare(salary) {
  if (salary < configs.medicareBrackets[0].start) {
    return 0;
  } else if (
    salary >= configs.medicareBrackets[0].start &&
    salary < configs.medicareBrackets[0].end
  ) {
    return AtoRound(
      decimalUtils.times(
        configs.medicareBrackets[0].rate,
        salary - configs.medicareBrackets[0].start
      ),
      0.01
    );
    ...
    return AtoRound(
      decimalUtils.times(configs.medicareBrackets[1].rate, salary),
      0.01
    );
  }
}
...
function CalculateSalaryBeforeTax(takeHome) {
  ...
  let lowestSalary = configs.lowestSalary;
  let highestSalary = configs.highestSalary;
